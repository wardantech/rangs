...
use App\Models\Requisition\ReceivedDetails;
...
class TechnicianAllocationReceivedController extends Controller
{
    public function index()
    {
        try{
            ...
            if($user_role->name == 'Technician') {
                ...
                $receives = BranchAllocationReceived::where('belong_to',3)->where('store_id', $mystore->id)
                ->latest()->get();
            }elseif($user_role->name == 'Admin' || $user_role->name == 'Super Admin' || $user_role->name =='Store Admin') {
                $receives = BranchAllocationReceived::where('belong_to',3)->latest()->get();
            }else {
            ...
            return view('allocation.technician.received.index', compact('receives'));
        } catch (\Exception $e) {
            $bug = $e->getMessage();
    ...
    public function requisitionTechnicianReceiveform($id)
    {
        try{
            $allocation = Allocation::with('requisition')->where('id', $id)->first();

            $details = AllocationDetails::where('allocation_id', $allocation->id)
                        ->with('part')->get();
            $racks = Rack::where('status', 1)->where('store_id', $allocation->store_id)->get();

            $stock_collect = $this->stockInHand($details, $allocation);

            return view('employee.requisition.receive_form', compact(
                'allocation', 'details', 'stock_collect','racks'
            ));
        } catch (\Exception $e) {
            $bug = $e->getMessage();
            return redirect()->back()->with('error', $bug);
    ...
    public function requisitionTechnicianReceiveStore(Request $request)
    {
        $this->validate($request, [
            'receiving_quantity' => 'required|array',
        ]);
        ...
        try {
            ...
            $data['belong_to'] = 3;
            $data['store_id'] = $request->to_store_id;
                    ...
                    if($received) {
                        ...
                        if($request->receiving_quantity[$key] > 0){
                            InventoryStock::create([
                                ...
                                'allocation_received_details_id' => $details->id,
                                'part_id' => $request->part_id[$key],
            ...
            DB::commit();
            return redirect()->route('technician.requisition.allocate.receive')
    ...
    {
        try{
            $received = BranchAllocationReceived::findOrFail($id);

            $stock_collect = $this->stockInHand($received->receivedDetails, $received->allocation);

            return view('allocation.technician.received.edit', compact(
    ...
    public function show($id)
    {
        $received = BranchAllocationReceived::with('allocation', 'receivedDetails')
        ->findOrFail($id);
        $receivedDetails =BranchAllocationReceivedDetali::where('branch_allocation_received_id', $received->id)
                            ->with('part')
                            ->get();

        return view('allocation.technician.received.show', compact(
            'received',
            'receivedDetails'
        ));
    }
