                ...
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            ...
                            <span>{{ __('label.LIST OF BRAND_MODEL')}}</span>
                    ...
                    <div class="card-header">
                        ...
                        @can('create')
                            <div class="card-header-right">
                                <a href="{{URL::to('product/brand_model/create')}}" class="btn btn-primary">  @lang('label.CREATE')</a>
                            </div>
                        @endcan
                    </div>
                    @can('create')
                        <div style="margin: 0 auto" class="mb-2">
                            <a href="{{route('product.sample-brand-model-excel')}}" class="btn btn-success">Sample Excel Download</a>
                        </div>
                        <div style="margin: 0 auto">
                            {{-- <a href="{{route('export-inventory')}}" class="btn btn-success">Export</a> --}}
                            <form action="{{route('product.import-brand-model')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div>
                            ...
                            <tbody>
                                {{-- @foreach ($brandmodels as $brandmodel)
                                    <tr>
                                        ...
                                        <td>{{$brandmodel->model_name}}</td>
                                        <td>{{$brandmodel->code}}</td>
                                        <td>{{$brandmodel->brand ? $brandmodel->brand->name : ''}}</td>
                                        <td>
                                            {{$brandmodel->category ? $brandmodel->category->name : ''}}
                                        </td>
                                        <td>
                                            @if ($brandmodel->status == true)
                                                <form action="{{ route('product.brand_model.status', $brandmodel->id) }}" method="POST">
                                                    @csrf
                                        ...
                                        <td>
                                            <div class='table-actions text-center'>
                                                @can('edit')
                                                    <a href="{{ route('product.brand_model.edit', $brandmodel->id) }}" title="Edit">
                                                        <i class='ik ik-edit f-16 mr-15 text-blue'></i>
                                                    </a>
                                                @endcan

                                                @can('delete')
                                                    <form action="{{ route('product.brand_model.destroy', $brandmodel->id) }}" method="POST" class="delete d-inline">
                                                        @csrf
                                                        @method('DELETE')
    ...
    <script type="text/javascript">
        $(document).ready( function () {
            ...
            var dTable = $('#table').DataTable({
                ...
                ajax: {
                    url: "{{route('product.brand_model.index')}}",
                    type: "get"
                },
                columns: [
