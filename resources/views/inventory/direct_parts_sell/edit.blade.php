                ...
                <div class="card ">
                    <div class="card-body">
                        {{-- <form class="forms-sample">
                            {{ csrf_field() }} --}}
                            {{Form::open(['route'=>array('sell.update.direct-parts-sell', $partSell->id), 'method'=>'POST', "class"=>"form-horizontal form-prevent-multiple-submits"])}}
                            <div class="row mb-2">
                                <div class="col-sm-4">
                                    <label for="date">{{__('label.MR_NO')}}</label>
                                    <div>
                                        <input type="text" class="form-control" id="mr_no" name="mr_no" value="{{$partSell->mr_no}}" placeholder="MR No" readonly>
                                    </div>
                                    @error('mr_no')
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <div>
                                        <input type="text" class="form-control" id="sales_by" name="sales_by" value="{{$partSell->sales_by}}" placeholder="Sales by" readonly>
                                        <input type="hidden" name="part_sell_id" id="part_sell_id" value="{{$partSell->id}}">
                                    </div>
                                    @error('sales_by')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <div>
                                        <input type="date" class="form-control" id="date" name="date" value="{{$partSell->date->format('Y-m-d')}}" placeholder="Date Name" required>
                                    </div>
                                    @error('date')
                                        <div class="text-danger">{{ $message }}</div>
                                    @enderror
                                    ...
                                    @if ($user_role->name == 'Super Admin' || $user_role->name == 'Admin')
                                        <select name="store_id" id="store" class="form-control" required>
                                            ...
                                            @foreach($stores as $store)
                                                <option value="{{$store->id}}" {{$store->id == $partSell->store_id ? 'selected' : ''}}>{{$store->name}}</option>
                                            @endforeach
                                        </select>
                                    @else
                            ...
                            <div class="row">
                                    ...
                                    <div>
                                        <select name="customer_id" id="customer_id" class="form-control" required>
                                            ...
                                            @foreach($customers as $customer)
                                                <option value="{{$customer->id}}" {{$customer->id == $partSell->customer_id ? 'selected' : ''}}>{{$customer->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('customer_id')
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <div>
                                        <input type="number" class="form-control" id="customer_phone" name="customer_phone" value="{{$partSell->customer_phone}}" placeholder="Customer Phone" readonly required>
                                    </div>
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <div>
                                        <textarea name="customer_address" id="customer_address" class="form-control" cols="12" rows="1" readonly required>{{$partSell->customer_address}}</textarea>
                                    </div>
