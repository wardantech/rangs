            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>{{ __('label.RACKS')}}</h5>
                            <span>{{ __('label.LIST OF RACKS')}}</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                            ...
                            <tbody>
                                ...
                                @foreach($racks as $rack)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($rack->status == true)
                                                <form action="{{ route('inventory.racks.status', $rack->id) }}" method="POST">
                                                    @csrf
                                        ...
                                        <td>
                                            <div class='text-center'>
                                                @can('edit')
                                                    <a href="{{ route('inventory.racks.edit', $rack->id) }}" class="show-outlet">
                                                        <i class='ik ik-edit f-16 mr-15 text-blue'></i>
                                                    </a>
                                                @endcan

                                                @can('delete')
                                                    <form action="{{ route('inventory.racks.destroy', $rack->id) }}" method="POST" class="delete d-inline">
                                                        @csrf
                                                        @method('DELETE')
    ...
    <script type="text/javascript">
        $(document).ready( function () {
            ...
            var dTable = $('#datatable').DataTable({
                ...
                ajax: {
                    url: "{{route('inventory.racks.index')}}",
                    type: "get"
                },
                columns: [
