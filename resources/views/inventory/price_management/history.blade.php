                        ...
                        <table id="datatable" class="table">
                            ...
                            <tbody>
                                ...
                                @foreach($pricemanagements as $priceManagement)
                                    <tr>
                                        <td>{{$i++}}</td>
                                        <td>{{$priceManagement->created_at->format('m/d/yy H:i:s')}}</td>
                                        <td>{{$priceManagement->part->code}}</td>
                                        <td>{{$priceManagement->part->name}}</td>
                                        <td>{{$priceManagement->part->partModel->name}}</td>
                                        <td>{{$priceManagement->cost_price_usd}}</td>
                                        <td>{{$priceManagement->cost_price_bdt}}</td>
                                        <td>{{$priceManagement->selling_price_bdt}}</td>
                                        <td>
                                            <div class='text-center'>
                                                    <a href="{{route('inventory.price-management.edit', $priceManagement->id)}}" class="show-priceManagement">
                                                        <i class='ik ik-edit f-16 mr-15 text-blue'></i>
                                                    </a>
                                                @can('delete')
                                                    <form action="{{ route('inventory.price-management.destroy', $priceManagement->id) }}" method="POST" class="delete d-inline">
                                                        @csrf
                                                        @method('DELETE')
