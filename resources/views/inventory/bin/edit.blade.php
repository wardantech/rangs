        ...
        @can('create')
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="card ">
                        <div class="card-body">
                            <form action="{{ route('inventory.bins.update', $bin->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col">
                                        ...
                                        <select name="store_id" id="store" class="form-control" required>
                                            ...
                                            @foreach($stores as $store)
                                                <option value="{{$store->id}}" {{$store->id==$bin->store_id ? 'selected' : ''}}>
                                                    {{$store->name}}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col">
                                        ...
                                        <select name="rack_id" id="rack" class="form-control" required>
                                            ...
                                            @foreach($racks as $rack)
                                                <option value="{{$rack->id}}" {{$rack->id==$bin->rack_id ? 'selected' : ''}}>{{$rack->name}}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row mt-3">
                                    <div class="col col-sm-6">
                                        ...
                                        <input type="text" class="form-control" id="bin-name" name="name" value="{{$bin->name}}" placeholder="Bin Name" required>
