...
@section('title', 'Allocated Received')
@section('content')
            ...
            <div class="row align-items-end">
                <div class="col-lg-8">
                    <div class="page-header-title">
                        ...
                        <div class="d-inline">
                            <h5>{{ __('Allocated Received')}}</h5>
                            <span>{{ __('Allocated Received list')}}</span>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                            ...
                            <tbody>
                                @foreach ($receives as $key=> $received)
                                    <tr>
                                        ...
                                        <td>JSL-{{ $received->allocation->requisition->job->id ?? null}}</td>
                                        <td>TSL-{{ $received->allocation->requisition->job->ticket->id ?? null}}</td>
                                        <td>
                                            {{ $received->allocation->requisition->requisition_no ?? null}}
                                        </td>
                                        ...
                                        <td>
                                            {{ $received->allocation->requisition->received_quantity ?? null}}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class='' style="display: flex;">
                                                @can('edit')
                                                    <a href="{{ route('technician.allocation.received.edit', $received->id) }}">
                                                        <i class="ik ik-edit f-16 mr-15 text-green"></i>
                                                    </a>
                                                @endcan

                                                @can('show')
                                                    <a  href="{{ route('technician.allocation.received.show', $received->id) }}">
                                                        <i class="ik ik-eye f-16 mr-15 text-blue"></i>
                                                    </a>
                                                @endcan

                                                @can('delete')
                                                    <form action="{{ route('technician.allocation.received.destroy', $received->id) }}" method="POST" class="delete d-inline">
                                                        @csrf
                                                        @method('DELETE')
