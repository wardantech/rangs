    ...
    <tbody>
        @foreach ($stock_collect as $key=>$stock)
            <tr>
                ...
                <td><input type="number" class="form-control" name="required_quantity[]" min="0" value="{{$collectRequiredQuantity[$key]}}"><input type="hidden" name="part_id[]" value="{{ $partInfo_collect[$key]->id }}"></td>
            </tr>
        @endforeach
    </tbody>
</table>
