...
<body onload="window.print();">
    ...
    <div style="margin-top:20px;" class="bill">
    ...
    <table border="1" class="t-details">
        <tr>
            ...
            <td>{{ $jobSubmission->created_at }}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td  class="emptyborder"></td>
            <td colspan="3">Model No : {{$jobSubmission->job->ticket->purchase->modelname->model_name}}</td>
        </tr>
        <tr>
            <td  class="emptyborder"></td>
            <td colspan="3">Serial No : {{$jobSubmission->job->ticket->purchase->product_serial}}</td>
        </tr>
        <tr>
            <td  class="emptyborder"></td>
            <td colspan="3">Job No : JSL-{{$jobSubmission->job->id}} & TSL-{{$jobSubmission->job->ticket->id}}</td>
        </tr>
    </table>
    <table style="margin-top: 50px;" class="memo-body" border="1">
        ...
        <tr>
            ...
            ?>
        <th>{{ number_format($jobSubmission->total_amount, 2) }}</th>
        </tr>
        <tr style="border: 0px;text-align:left;">
            @if ($jobSubmission->total_amount)
            <?php $get_amount= numberTowords($jobSubmission->total_amount); ?>
            <th colspan="6">Total Amount in Word: {{ $get_amount }}</th>
            @else
...
function numberTowords(float $amount)
{
  ...
  while( $x < $count_length ) {
   ...
   $get_paise = ($amount_after_decimal > 0) ? "And Cents " . ($change_words[$amount_after_decimal / 10] . "
