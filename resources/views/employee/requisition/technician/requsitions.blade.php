                        ...
                        <table id="datatable" class="table">
                            <thead>
                                <tr>
                                    <th>{{ __('label.SL')}}</th>
                                    <th>{{ __('label.DATE')}}</th>
                                    <th title="Technician Name">{{ __('Raised By')}}</th>
                                    <th title="Requisition No">{{ __('Req No')}}</th>
                            ...
                            <tbody>
                                @foreach ($requisitions as $key=>$requisition)
                                    <tr>
                                        ...
                                        <td>{{ $requisition->store->name ?? null }}</td>
                                        ...
                                        <td>
                                            <div class='' style="text-align: right;">
                                                @if ($requisition->allocation_status!=1)
                                                    @can('create')
                                                        <a  href="{{ route('branch.requisition-allocate',$requisition->id) }}"  title="Allocate" data-id="{{$requisition->id}}">
                                                            <i class="fa fa-reply f-16 mr-15" aria-hidden="true"></i>
                                                        </a>
                                                    @endcan
                                                @else
                                                    @can('create')
                                                        <i class="fa fa-reply f-16 mr-15 text-yellow" aria-hidden="true" title="Already Allocated"></i>
                                                    @endcan
                                                @endif
                                                @can('show')
                                                    ...
                                                    <a href="{{ route('branch.technician-requisitions.show', $requisition->id) }}" title="View Details">
                                                        <i class="fas fa-binoculars f-16 mr-15 text-green"></i>
                                                    </a>
                                                @endcan

                                                {{-- @if ($requisition->allocation_status!=1)
                                                    <a  href="{{ route('branch.requisitions.decline',$requisition->id) }}"  title="Decline" data-id="{{$requisition->id}}">
                                                        <i class="fa fa-times text-red" aria-hidden="true"></i>
                                                    </a>
                                                @else
                                                    <i class="fa fa-times text-yellow" aria-hidden="true" title="Already Allocated"></i>
                                                @endif --}}
                                            </div>
                                        </td>
                                    </tr>
