                            ...
                            <tbody>
                                @foreach ($allocations as $key=>$allocation)
                                    <tr>
                                        ...
                                        <td>{{ $allocation->requisition->requisition_no ?? null}}</td>
                                        <td>{{ $allocation->requisition->senderStore->name ?? null}}</td>
                                        <td>{{ $allocation->requisition->total_quantity ?? null}}</td>
                                        <td>{{ $allocation->requisition->received_quantity ?? null}}</td>
                                        <td>{{ $allocation->allocate_quantity ?? null}}</td>
                                        ...
                                        <td>
                                            ...
                                            @elseif($allocation->status == 1 && $allocation->requisition->total_quantity > $allocation->allocate_quantity)
                                               <a href="{{ route('central.re-allocate', $allocation->requisition->id) }}" class="badge badge-warning" title="Re Allocate">
                                                   Partially Allocated
                                        ...
                                        <td>
                                            <div class='' style="text-align: right;">
                                                @can('edit')
                                                    ...
                                                    <a  href="{{ route('central.allocation.edit', $allocation->id) }}">
                                                ...
                                                @can('show')
                                                    <a  href="{{ route('central.allocation.show', $allocation->id) }}">
                                                ...
                                                @can('delete')
                                                    @if ($allocation->is_received != 1)
                                                        <form action="{{ route('central.allocation.destroy', $allocation->id) }}" method="POST" class="delete d-inline">
                                                            @csrf
                                                            @method('DELETE')
